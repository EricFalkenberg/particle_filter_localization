...
vector<MyPoint> path_compression(vector<MyPoint> path, Mat image)
{
    ...
    for(;;)
    {
        ...
        if(slope(path[i-1], path[i])==slope(path[i], path[i+1]) \
    ...
    for (auto temp : path)
    {
        ...
        cout << temp.x/CONVFACTOR << ", " << temp.y/CONVFACTOR << endl;
    }
    return path;
}
    ...
    while(!openset.empty())
    {
        ...
        for (auto n : nlist)
        {
                ...
                tentative_g_score<g_score[n])
            {
                ...
                f_score[n]=3*g_score[n] + 3*heuristic(n, goal);
...
int main()
{
    auto shit = astar(MyPoint(171, 201), MyPoint(-189, 189));
}